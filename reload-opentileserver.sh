...
#       reload-opentileserver.sh -add pbf_url	# keep old and add new pbf
#	reload-opentileserver.sh pbf_url	# remove old and add new pbf
#	reload-opentileserver.sh 		# remove old and add default pbf
#	reload-opentileserver.sh -h		# print usage
...
if [ "${1}" == '-h' ]; then
	echo "Usage: $0 [-add] [pbf_url]";
	exit 1;
fi

DEFAULT_PBF_URL='http://download.geofabrik.de/asia/taiwan-latest.osm.pbf'

PBF_URL="${1}";	#http://download.geofabrik.de/asia/taiwan-latest.osm.pbf
OSM_USER='tile'
OSM_DB='gis'
...
if [ "${PBF_ADD}" == 'no' ]; then
	echo "dropdb ${OSM_DB}, dropuser ${OSM_USER}"
...
create database ${OSM_DB} TEMPLATE template0 ENCODING 'UTF8' owner=${OSM_USER};
\c ${OSM_DB}
...
echo "sudo -u ${OSM_USER} osm2pgsql ${osm2pgsql_OPTS} ${PBF_FILE}"
